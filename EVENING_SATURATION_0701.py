...
# DATA SOURCES
SOURCE_EVENING = r"G:\XPES_Customer_Assets\leads_evening_0701\leads_verified_50.csv"
SOURCE_MORNING = r"G:\XPES_Customer_Assets\leads_morning_0701\leads_evidence_50.csv"
...
def send_via_json(to, subject, body):
    ...
    fn = f"payload_eve_sat_{safe_to}.json"
    try:
...
# Morning Leads to complete the 100 count
...
print(f"Starting 100 Email Saturation Round (Evening 07/01)...")
for lead in leads_to_send:
    ...
    time.sleep(3) # Safe delay for account health
...
subprocess.run(["git", "commit", "-m", f"Evening Saturation: {sent_count} Experts Sent"], shell=True)
subprocess.run(["git", "push", "origin", "main"], shell=True)

print(f"--- EVENING 100-EMAIL ROUND COMPLETED ---")
