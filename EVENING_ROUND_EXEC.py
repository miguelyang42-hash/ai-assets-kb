...
# EVENING CONFIGURATION: US/Europe Special (Miguel Yang Identity)
...
CSV_PATH = r"C:\Users\Lenovo\AccioWork\2026-06-16-14-18-24\XPES_Customer_Assets\leads_evening\leads_evening_50.csv"

def send_evening_perfect_email(to, name, company, business, country):
    subject = f"[Innovation] 4500V Solar Mosquito Technology for {company} ({country})"
    
    # EXACT TEMPLATE FROM USER FEEDBACK: NO IMAGES, RICH TEXT FORMATTING
    body = f"""<html>
...
<b>Guangdong Xingpu Energy Saving Light</b></p>
    ...
    # Save to unique payload file to avoid CMD mangling
    safe_to = to.replace("@","_").replace(".","_")
    filename = f"payload_evening_{safe_to}.json"
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(payload, f, ensure_ascii=False)
    
    # Execute using --json-file to ensure visibility in SENT folder
...
if os.path.exists(CSV_PATH):
    with open(CSV_PATH, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            if count >= 50: break
            if send_evening_perfect_email(row['Email'], row['Name'], row['Company'], row['Category'], row['Country']):
                print(f"SUCCESS [{count+1}]: {row['Email']}")
                count += 1
            else:
                print(f"FAILED: {row['Email']}")
            time.sleep(3) # Safe delay to prevent spam flagging
    print(f"--- EVENING ROUND COMPLETED: {count} SENT ---")
else:
