...
mcp_cli = r"C:\Users\Lenovo\AppData\Roaming\Accio\pre-install\e6550f7e00ff\accio-mcp-cli.cmd"
...
for r in recipients:
    ...
    cmd = [mcp_cli, "call", "send_gmail_message", "--json", json.dumps(payload)]
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode == 0:
        print(f"  Success: {r['to']}")
    else:
        print(f"  Failed: {r['to']} - {result.stderr}")
    time.sleep(1)
