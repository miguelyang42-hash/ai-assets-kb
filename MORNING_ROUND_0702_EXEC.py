...
# CONFIGURATION FOR 07/02 MORNING ROUND
...
CSV_PATH = r"G:\XPES_Customer_Assets\leads_morning_0702\leads_evidence_50.csv"
...
def send_via_json_file(to, subject, body):
    ...
    filename = f"payload_morn_0702_{safe_to}.json"
    try:
...
if os.path.exists(CSV_PATH):
    ...
    subprocess.run(["git", "commit", "-m", f"07/02 Morning Round: {sent_count} Verified Experts Sent"], shell=True)
    subprocess.run(["git", "push", "origin", "main"], shell=True)
    
    print(f"--- 07/02 MORNING ROUND COMPLETE: {sent_count} SENT & SYNCED ---")
else:
