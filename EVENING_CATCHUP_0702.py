...
# CONFIGURATION FOR EVENING SATURATION (100 EMAILS)
...
# DATA SOURCES
SOURCE_EVENING = r"G:\XPES_Customer_Assets\leads_evening_0702\leads_verified_50.csv"
# For catch-up, we use the afternoon batch or extra from master not sent yet
SOURCE_CATCHUP = r"G:\XPES_Customer_Assets\leads_afternoon_0702\leads_verified_50.csv"
...
def send_via_json(to, subject, body):
    ...
    filename = f"payload_eve_{safe_to}.json"
    try:
...
# Source Catch-up (completing to 100)
if os.path.exists(SOURCE_CATCHUP):
    with open(SOURCE_CATCHUP, mode='r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for row in reader:
...
print(f"Starting 100 Email Expert Sourcing Round (Evening 07/02)...")
for lead in leads_to_send:
    to = lead['Email']
    name = lead['Name']
    company = lead['Company']
    biz = lead.get('Category', 'Outdoor/Garden')
...
subprocess.run(["git", "commit", "-m", f"Evening Saturation 07/02: {sent_count} Experts Sent & Verified"], shell=True)
