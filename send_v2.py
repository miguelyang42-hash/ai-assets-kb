...
def send_email(lead):
    ...
    # Write payload to a temp file to avoid CLI argument length issues and escaping
    with open('tmp_p.json', 'w', encoding='utf-8') as f:
        json.dump(p, f)
    
    print(f"Sending to {lead['to']}...")
    # Use Powershell to read the JSON and call the CLI
    # This bypasses the < > issues in batch files
    ps_cmd = f'$json = Get-Content tmp_p.json -Raw; & "{cli}" call send_gmail_message --json $json'
